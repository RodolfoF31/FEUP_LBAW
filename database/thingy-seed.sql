...
-- Create Product Table (before tables that depend on it)
...
-- Now create tables that reference the product table
...
CREATE FUNCTION notify_cart_product_price_change() RETURNS TRIGGER AS
$BODY$
DECLARE
    cart_record RECORD;
    notification_id BIGINT;
BEGIN
    IF NEW.price <> OLD.price THEN
        FOR cart_record IN
            SELECT sc.id_authenticated_user, sc.id AS id_shopping_cart
            FROM "shoppingCartProduct" scp
            JOIN "shoppingCart" sc ON sc.id = scp.id_shopping_cart
            WHERE scp.id_product = NEW.id
        LOOP
            INSERT INTO "notification" (id_authenticated_user, text)
            VALUES (cart_record.id_authenticated_user, 'The price of product ' || NEW.title || ' has changed to ' || NEW.price || '!');

            notification_id = currval('notification_id_seq');

            INSERT INTO "productOnCartPriceChangeNotification" (id_shopping_cart, id_notification)
            VALUES (cart_record.id_shopping_cart, notification_id);
        END LOOP;
    END IF;
...
BEGIN
    ...
    IF NEW.orderstatus <> OLD.orderstatus THEN
        -- Insert a new notification for the user whose order status changed
        INSERT INTO "notification" (id_authenticated_user, text)
        ...
        -- Insert into the linking table for changeOrderStatusNotification
        INSERT INTO "changeOrderStatusNotification" (id_order, id_notification)
        VALUES (NEW.id, notification_id);
    END IF;

    -- Return the new row after processing
    RETURN NEW;
END;
$BODY$
LANGUAGE plpgsql;
...
-- Create the trigger on the orders table for when the status is updated
